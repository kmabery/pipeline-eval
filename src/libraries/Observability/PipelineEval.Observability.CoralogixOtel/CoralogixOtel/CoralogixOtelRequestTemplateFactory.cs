...
/// <summary>
/// Vendored from serilog/serilog-sinks-opentelemetry v4.2.0 (Apache-2.0).
/// </summary>
static class CoralogixOtelRequestTemplateFactory
{
    ...
    static RepeatedField<KeyValue> ToResourceAttributes(IReadOnlyDictionary<string, object> resourceAttributes)
    {
        ...
        {
            var v = CoralogixOtelPrimitiveConversions.ToOpenTelemetryPrimitive(entry.Value);
            var kv = new KeyValue
            {
                Value = v,
                Key = entry.Key
